/*****************************************************************************/		//Falta modificar la funcio pilotes i que se creen 												els diferents threads al main. I PROVAR!
/*                                                                           */
/*                           Fronton0.c                                      */
/*                                                                           */
...
int f_pil[PILOTA_THREADS], c_pil[PILOTA_THREADS];	/* posicio de la pilota, en valor enter */
float pos_f[PILOTA_THREADS], pos_c[PILOTA_THREADS];	/* posicio de la pilota, en valor real */
float vel_f[PILOTA_THREADS], vel_c[PILOTA_THREADS];	/* velocitat de la pilota, en valor real */
int retard;		/* valor del retard de moviment, en mil.lisegons */

char strin[65];		/* variable per a generar missatges de text */

int fi1, fi2;		/* convertim les condicions de fi en globals*/
...
int inicialitza_joc(void)
{
  ...
  for(i=0; i<PILOTA_THREADS; i++){
	  ...
	  win_escricar(f_pil[i],c_pil[i],i+1,INVERS);
  }
...
do
{
  ...
  {		/* si posicio hipotetica no coincideix amb la posicio actual */
	...
	{    vel_f[index] = -vel_f[index]; vel_c[index] = -vel_c[index];	/* canvia sentit velocitats */
	...
	pos_f[index] += vel_f[index]; pos_c[index] += vel_c[index];
	f_pil[index] = f_h; c_pil[index] = c_h;		/* actualitza posicio actual */
	if (c_pil[index] != 0)		 		/* si ho surt del taulell, */
		win_escricar(f_pil[index],c_pil[index],'index'+1,INVERS); /* imprimeix pilota */
	else
		fi2=1;		/* codi de finalitzacio de partida */
    }
  }
  else { pos_f[index] += vel_f[index]; pos_c[index] += vel_c[index]; }
  win_retard(retard); 
}while (!fi1 && !fi2);
}
...
int main(int n_args, char *ll_args[])
{
  ...
  //pthread_create(&tidPilota, NULL, mou_pilota, 1);
