...
int id_sem, *id_bustia_m, id_bustia_mem;
char id_semc[10], id_bustia_s[10];
...
sprintf(id_semc,"%i",id_sem);	 /* convertir identificador sem. en string */

////////////////////////////////////////////////////////////////////////////////////////////
id_bustia_mem = ini_mem(pilotes*sizeof(int));
id_bustia_m = map_mem(id_bustia_mem);

for(i=0; i<pilotes; i++){
	id_bustia_m[i] = ini_mis();	/* crear bustia IPC*/
}

sprintf(id_bustia_s, "%i", id_bustia_mem);	    	
////////////////////////////////////////////////////////////////////////////////////////////
...
int main(int n_args, char *ll_args[])
{
  ...
  for ( i = 0; i < pilotes; i++)
  {
	...
	execlp("./pilota4", "pilota4", num_pilota, mida_camp, files_camp, col_camp, retards, pos_f, pos_c, vel_f, vel_c, f_pil, c_pil, id_fi1c, id_fi2c, id_fiRebotsc, max_rebots, id_semc, id_bustia_s, (char *)0);
  ...
  elim_mem(id_fi1);
  elim_mem(id_fi1);
  elim_mem(id_fiRebots);
  elim_sem(id_sem);
  for(i=0; i<pilotes; i++){
	elim_mis(id_bustia_m[i]);	/* elimina bustia */
  }
